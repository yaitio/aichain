"""
tests.clients.test_private_provider
===================================

The private provider: a model server you control, asserted on the wire.
...
though the server runs open; and the base URL every such server's docs quote
...
What is *not* asserted here is privacy: the provider will happily talk to a
hosted aggregator, and nothing in the code can tell the difference. The word
names the intent of a configuration, and the docs carry that caveat.
...
class TestRouting(unittest.TestCase):
    ...
    def test_hf_id_keeps_its_slashes(self):
        ...
        """
        m = Model("private/meta-llama/Llama-3.3-70B-Instruct")
        self.assertEqual(m._provider, "private")
...
class TestBaseURL(unittest.TestCase):
    """
    Every such server's docs quote the base URL with /v1 — Ollama's
    ...
    def test_documented_v1_urls_do_not_double(self):
        for url in ("http://localhost:11434/v1",
            ...
            with self.subTest(url=url):
                m = Model("private/qwen3-30b", client_options={"url": url})
                full, _ = request_of(m)
                self.assertNotIn("/v1/v1/", full)
                self.assertTrue(full.endswith("/v1/chat/completions"))

    def test_bare_host_gets_the_path(self):
        m = Model("private/qwen3-30b", client_options={"url": "http://gpu-box:8000"})
        full, _ = request_of(m)
        self.assertEqual(full, "http://gpu-box:8000/v1/chat/completions")

    def test_env_var_points_at_a_remote_box(self):
        os.environ["PRIVATE_BASE_URL"] = "http://gpu:9000"
        try:
            m = Model("private/qwen3-30b")
            self.assertTrue(request_of(m)[0].startswith("http://gpu:9000/"))
        finally:
            del os.environ["PRIVATE_BASE_URL"]

    def test_explicit_url_beats_the_env_var(self):
        os.environ["PRIVATE_BASE_URL"] = "http://wrong:9000"
        try:
            m = Model("private/qwen3-30b", client_options={"url": "http://right:8000"})
            self.assertTrue(request_of(m)[0].startswith("http://right:8000/"))
        finally:
            del os.environ["PRIVATE_BASE_URL"]

    def test_an_aggregator_endpoint_works_and_is_not_special_cased(self):
        """
        Pointing "private" at a hosted aggregator is supported and, from the
        code's side, indistinguishable from a server of your own — which is
        exactly why the privacy claim lives in prose the reader can weigh
        rather than in a name the library pretends to enforce. Pinned so
        nobody later "fixes" it by blocking public hosts: the choice of
        endpoint belongs to the caller.
        """
        m = Model("private/meta-llama/Llama-3.3-70B-Instruct",
                  api_key="aggregator-key",
                  client_options={"url": "https://api.together.xyz/v1"})
        full, body = request_of(m)
        self.assertEqual(full, "https://api.together.xyz/v1/chat/completions")
        self.assertEqual(body["model"], "meta-llama/Llama-3.3-70B-Instruct")
        self.assertEqual(m.client._auth_headers()["Authorization"],
                         "Bearer aggregator-key")
