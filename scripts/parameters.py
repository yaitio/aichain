...
def _defaults_section() -> list:
    ...
    lines += [
        ...
        "and would change the output of every existing caller in order to fix "
        "a comparison only some of them are making. What the library owes is "
        "the fact, not a decision — `Model.effective_options` returns exactly "
        "what this model will run with, asked for or not, so a measurement "
...
def render_doc(matrix: dict) -> str:
    lines = [
        ...
        "",
        "Four questions, one source. **What you get without asking**, "
        "**what you may ask for**, **what each "
        ...
        "",
        "**When an option cannot be honoured, the request still goes and the "
        "library says what it dropped.** That is the default because a "
        "library which raises on a provider gap is one you cannot swap a "
        "provider under. It is not always the right default for the caller: "
        "dropping `temperature` gives a different answer, but dropping "
        "`seed`, `size`, `background`, `fidelity`, `output_format` or "
        "`compression` gives a result that *looks* right and is not. So the "
        "choice is the caller\u2019s — `Model(..., on_unsupported=...)` takes "
        "`'warn'` (the default), `'requirements'` (raise only for that second "
        "class) or `'raise'` (any option that did not reach the wire). A "
        "measurement arm should set the last one; the record is written to "
        "`Model.last_adaptations` either way, before the raise.",
        "",
    ]
    lines += _defaults_section()
